...

{
  ...
  echo "top_n=$TOP_N max_runtime_minutes=$MAX_RUNTIME batch_size=$BATCH_SIZE materialize_batch_size=$MATERIALIZE_BATCH_SIZE factor_top_n=$FACTOR_TOP_N"
  ...
  python3 edge_lab_materialize.py --db data/edge_lab.db --batch-size "$MATERIALIZE_BATCH_SIZE"
  python3 edge_lab_factors.py --db data/edge_lab.db --out data/factor_report.json --top-n "$FACTOR_TOP_N" --quiet
